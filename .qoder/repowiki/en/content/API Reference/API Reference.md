...
## Introduction
This API reference documents the public interfaces of the VIZ PHP Library, focusing on the Key, Transaction, JsonRPC, Auth, and Utils classes. It provides method specifications, parameter types, return values, exceptions, usage examples, integration patterns, error handling strategies, and performance considerations. The goal is to enable developers to integrate with the VIZ blockchain using native PHP classes for cryptographic operations, transaction building, JSON-RPC communication, authentication, and utility helpers.

## Project Structure
The library follows a PSR-4-compatible structure with a classmap fallback and autoloading support. The primary namespace is VIZ, with supporting namespaces for elliptic curve cryptography and big integer arithmetic.
...
## Core Components
This section summarizes the responsibilities and public interfaces of each core class.

- Key: Cryptographic key management, signing, verification, shared key derivation, memo encryption/decryption, and key encoding/decoding.
...
- Auth: Passwordless authentication verification against VIZ chain authorities.
...
The library integrates cryptographic primitives, transaction builders, and JSON-RPC clients to interact with VIZ nodes. The Transaction class composes JsonRPC and Key to construct, sign, and broadcast transactions. Auth verifies signatures against chain authority configurations. Utils provides reusable helpers for encoding, encryption, and Voice protocol operations.
...
### Key Class API
The Key class manages private/public keys, shared key derivation, signing, verification, signature recovery, and memo encryption/decryption. It supports importing from WIF, hex, binary, and public key encodings, and can encode keys for various prefixes.
...
- auth(account, domain, action, authority)
  ...
  - Behavior: Generates signed challenge data for passwordless auth.
...
### Auth Class API
The Auth class verifies passwordless authentication challenges against VIZ chain authority thresholds.
...
Usage examples and integration patterns are demonstrated in the README under Auth section.
...
## Dependency Analysis
The following diagram shows the primary dependencies among the core classes.
...
- Authentication verification fails
  ...
  - Causes: Out-of-range timestamps, wrong domain/action/authority, missing or insufficient key weights.
  - Resolution: Align server timezone offset, verify domain/action/authority, and ensure sufficient key weights.
...
## Conclusion
The VIZ PHP Library provides a cohesive set of classes for cryptographic operations, transaction construction, JSON-RPC communication, authentication, and utility functions. By leveraging the APIs documented here, developers can integrate with the VIZ blockchain efficiently, handle multi-signature scenarios, and implement advanced features like Voice protocol posts and memo encryption.
...
- Auth: Passwordless authentication verification against chain authorities.
