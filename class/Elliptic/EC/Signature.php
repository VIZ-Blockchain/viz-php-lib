...
class Signature
{
    ...
    function __construct($options, $enc = false)
    {
        ...
        if (!$this->_importCompact($options, $enc))
    ...
    private function _importCompact($data, $enc)
    {
        $data = Utils::toArray($data, $enc);
        $dataLen = count($data);
        ...
        $recoveryParam = $data[0];
        $recoveryParam-=27;//compact 24 or 27 (forcing odd-y 2nd key candidate)
        if($recoveryParam !== $recoveryParam & 7){
            return false;
        }
        $recoveryParam-=4;//compressed
        if($recoveryParam<0){
            return false;
        }
        $r = array_slice($data, 1, 32);
        $s = array_slice($data, 33, 32);
    ...
    public function toCompact($enc = false)
    {
        $der=self::toDER($enc);
        $der_bin=hex2bin($der);
        $lenR=hexdec(substr($der,6,2));
        $lenS=hexdec(substr($der,10+$lenR*2,2));
        if(32 == $lenR && 32 == $lenS){
            $recoveryParam = $this->recoveryParam;
            $res=dechex($this->recoveryParam + 4 + 27);
            $res.=substr($der,8,64);
            $res.=substr($der,-64);
            if('hex'!==$enc){
                $res=hex2bin($res);
